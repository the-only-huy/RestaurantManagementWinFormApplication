...
namespace RestaurantManagement
{
    public partial class frmTableManager : Form
    {
        ...
        private void btnCheckOut_Click(object sender, EventArgs e)
        {
            ...
            double totalPrice = Convert.ToDouble(txbTotalPrice.Text.Split('$')[1]);
            double finalTotalPrice = totalPrice - (totalPrice / 100) * discount;
            if (idBill != -1)
            {
                if (MessageBox.Show(string.Format("Bạn có chắc chắn muốn thanh toán hóa đơn cho {0}\n Tổng tiền - Giảm giá = {1} - ({1} / 100) * {2} = {3}", table.Name, totalPrice, discount, finalTotalPrice), "Thông báo", MessageBoxButtons.OKCancel) == DialogResult.OK)
                {
